...
class SUT:
    ...
    def __init__(self, sut_folder: Optional[Path]=None, device: Optional[str]=None):
        ...
        if sut_folder_name.split('/')[-1] == 'AAA_Original_000':
            temp_weights = torch.load(sut_folder / 'model_fuzzing.pth')
            self.net.load_state_dict(temp_weights)
        self.net.eval()
        self.transform = tf.Compose([tf.ToPILImage(), tf.Resize((28, 28)), tf.Grayscale(num_output_channels=1), tf.ToTensor()])
    ...
    def execute(self, images: Union[torch.Tensor, Iterable[np.array]]) -> List[Tuple[int, float, List[float]]]:
        with torch.no_grad():
            ...
            full_out = [(cls.item(), p.item(), out.detach().cpu().numpy().tolist()) for (cls, p, out) in zip(classes, probabilities, outputs)]
            return full_out
...
#        r = s.execute([img])
    import matplotlib.pyplot as plt
    filename_1 = np.array(PIL.Image.open(urllib.request.urlopen(sys.argv[1])))
    img = plt.imread(filename_1)
