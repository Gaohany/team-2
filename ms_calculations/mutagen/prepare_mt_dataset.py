...
sys.path.append("E:/code/fdr_ms_calculations_liu/fdr_ms_calculations")
...
def arguments():
    ...
    parser.add_argument(
        ...
        ###
        ####
        default="E:/code/fdr_ms_calculations_liu/fdr_ms_calculations/config/gtsrb.toml",
    )
...
def export(root_folder: Path, filename: str, dataset_list: list, num_classes: int):
    ...
    obj = {
        "img_failed": dataset_list[0],
        "lbl_failed": dataset_list[1],
        "path_failed": dataset_list[2],
        "ori_img_failed": dataset_list[3],
...
def load_ori_image(img_paths, config, preprocess):
    ...
    for path in tqdm(img_paths):
        if config.data == "gtsrb-gray":
            image = Image.open(Path(f"E:/datasets/{config.detection_model.image_set}_dataset/Test/{path:0{5}}.png"), mode="r").convert("L")
        elif config.data == "mnist":
            data_file = "E:/Code/fdr_ms_calculations_liu/fdr_ms_calculations/data/mnist/mnist_org_data_normalized.pth"
            dataset = torch.load(data_file, "cuda")
            image = dataset["img_test"][int(path)]
        else:
            image = Image.open(Path(f"E:/datasets/{config.detection_model.image_set}_dataset/Test/{path:0{5}}.png"), mode="r").convert("RGB")
...
if __name__ == "__main__":
    ...
    for i, selected_path in enumerate(experiment_paths):
        ...
        if (
            ...
            gt_labels = torch.tensor(np.array(dataset.classes)).squeeze().to(torch.uint8)
            img_paths = dataset.image_ids
            img_paths = np.array(img_paths)
            img_paths = torch.tensor(img_paths)
    ...
    export(
        ...
        ####
        #####
        filename="gtsrb_mt_failed_data_normalized.pth",
