...
class DuckDbAdapter(HarlequinAdapter):
    ...
    def __init__(
        ...
        **_: Any,
        ...
        try:
            ...
            self.udf_path = Path(udf_path).resolve() if udf_path is not None else None
    ...
    def connect(self) -> DuckDbConnection:
        ...
        if self.udf_path and self.udf_path.exists():
            spec = importlib.util.spec_from_file_location(f"udf.init.{self.udf_path.name}", self.udf_path)
            init_module = importlib.util.module_from_spec(spec)
            sys.modules[f"udf.init.{self.udf_path.name}"] = init_module
            spec.loader.exec_module(init_module)
            init_module.register(connection)
