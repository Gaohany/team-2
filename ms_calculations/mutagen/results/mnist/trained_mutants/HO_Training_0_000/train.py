# Applied mutation op ChangeLoss with args: dict_items([('loss', 'NLLLoss')])
# Applied mutation op ChangeOptimizer with args: dict_items([('optimizer', 'SGD'), ('extra_arg', 'momentum:0')])
...
def train(net: nn.Module, data: Dict[str, torch.tensor], out_file: Path, num_epochs: int=20, **kwargs):
    start = time.time()
    ...
    optimizer = optim.SGD(net.parameters(), lr=1.0, momentum=0)
