# Applied mutation op ChangeScheduler with args: dict_items([('scheduler', 'ConstantLR'), ('extra_arg', 'factor:0.1|total_iters:10')])
...
def train(net: nn.Module, data: Dict[str, torch.tensor], out_file: Path, num_epochs: int=20, **kwargs):
    ...
    scheduler = lr_scheduler.ConstantLR(optimizer, factor=0.1, total_iters=10)
    history = []
    val_history = []
