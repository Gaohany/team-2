...
class SUT:
    ...
    def __init__(self, sut_folder: Optional[Path]=None, device: Optional[str]=None):
        ...
        self.net.eval()
        self.transform = tf.Compose([tf.ToPILImage(), tf.Resize((32, 32)), tf.Grayscale(num_output_channels=1), tf.RandomEqualize(p=1.0), tf.ToTensor(), tf.Normalize((0.5317,), (0.3174,))])
