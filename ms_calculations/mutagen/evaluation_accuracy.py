...
def create_tensor_dataset(path: Path, split: str, has_path: bool, config):
    if split == "mnist":
        tensor_data = MNISTDataset(
                config=config, image_set=config.detection_model.image_set, mode=type, augmentation=False
            )
    elif split == "gtsrb":
        tensor_data = GTSRBDataset(
                config=config, image_set=config.detection_model.image_set, mode=type, augmentation=False
            )
    data = {}
    for i in range(0, len(tensor_data)):
        if i == 0:
            img, lbl = tensor_data.__getitem__(i)
            data[f"img_{split}"]=torch.tensor(img)
            data[f"lbl_{split}"]= torch.tensor(lbl)
    
        else:
            #print(i)
            img, lbl = tensor_data.__getitem__(i)
            torch.cat((data[f"img_{split}"], torch.tensor(img)), dim = 0)
            torch.cat((data[f"lbl_{split}"], torch.tensor(lbl)), dim = 0)
...
def run_eval(mutant: Mutant, data, dataset_name: str, config):
    start = time.time()
    print(
...
def eval(result_path: Path, model: str, args: argparse.ArgumentParser):
    ...
    if model == "mnist":
        data_sets = {
            "test": (
                Path("E:/datasets/mnist_dataset/mnist_test.pth"),
                "test",
            ),
        }
    elif model == "gtsrb":
    ...
    for di, (dataset_name, (dataset_path, split)) in enumerate(data_sets.items()):
        
        #######################################################
        data_set = create_tensor_dataset(dataset_path, split, has_path = False, config = config) 
    ...
    for di, (dataset_path, mutant_list) in enumerate(mutants_datasets.items()):
        data_set = create_tensor_dataset(dataset_path, split="train", has_path=False, config = config)
