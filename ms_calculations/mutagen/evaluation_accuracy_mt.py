...
def run_eval(mutant: Mutant, data: Data, dataset_name: str, config):
    ...
    with mutant.sut:
        for img, label, sample_ids, ori_img, i, end in batched(data, batch_size=2048):
            ...
            if dataset_name == "passed" or dataset_name == "failed":
                ori_p, ori_classes, ori_outputs = mutant.sut.execute_raw(ori_img.float())
            else:
                ori_p, ori_classes, ori_outputs = mutant.sut.execute_raw(ori_img.float())
            classes = classes.cpu()
...
def eval(result_path: Path, model: str, args: argparse.ArgumentParser):
    ...
    if model == "mnist":
        data_sets = {
            "passed": (
                Path("./data", "mnist", "mnist_mt_2_normalized.pth"),
                "passed"
            ),
            "failed": (
                Path("./data", "mnist", "mnist_mt_2_normalized.pth"),
        ...
        data_sets = {
            "passed": (
                Path("./data", "gtsrb", "gtsrb_mt_2_data_normalized.pth"),
                "passed",
            ),
            "failed": (
                Path("./data", "gtsrb", "gtsrb_mt_failed_data_normalized.pth"),
                "failed",
            ),
        }
    ...
    for di, (dataset_name, data_set_args) in enumerate(data_sets.items()):
        data_set = create_tensor_dataset(*data_set_args)
