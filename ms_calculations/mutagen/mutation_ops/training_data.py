...
def make_classes_overlap(data: Dict[str, torch.Tensor], percentage: float):
    ...
    counts = torch.unique(labels_squeezed, return_counts=True)
    
    d = sorted([(count, lbl) for lbl, count in zip(counts[0], counts[1])])
   
    mc_indices = (labels_squeezed == d[-1][1]).nonzero(as_tuple=False)
   
    n = round(len(mc_indices) * percentage)
    
    selected_indices = mc_indices[torch.randperm(len(mc_indices))[:n]]
    
    o_shape = list(labels.shape)
    new_shape = [n] + o_shape[1:]
    new_labels = labels.new_full(new_shape, d[-2][1])
...
def change_labels(data: Dict[str, torch.Tensor], percentage: float, which_label: int = 0):
    ...
    labels_squeezed = tensor.squeeze()
    change_indices = torch.nonzero(labels_squeezed == which_label, as_tuple=False)
    ...
    new_shape = [num_values_to_change] + o_shape[1:]
    new_values = torch.randint(0, num_classes, size=new_shape, dtype=torch.uint8, device=tensor.device)
